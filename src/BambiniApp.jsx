...
/* ═══════════════════════════════════════════════════════════════
   🧠 BRAIN INFOGRAPHIC — STILE "VINTAGE MEDICAL ILLUSTRATION"
═══════════════════════════════════════════════════════════════ */
...
const BRAIN_VISUALS = {
  frontal: { fill: "#EBD1D8", stroke: "#A17882", label: "Lobo Frontale", icon: "🧠" },
  parietal: { fill: "#D1DBEB", stroke: "#788BA1", label: "Lobo Parietale", icon: "✋" },
  temporal: { fill: "#EBE4D1", stroke: "#A19478", label: "Lobo Temporale", icon: "👂" },
  occipital: { fill: "#D1EBD1", stroke: "#78A178", label: "Lobo Occipitale", icon: "👁️" },
  cerebellum: { fill: "#DFD1EB", stroke: "#8E78A1", label: "Cervelletto", icon: "⚖️" }
};
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: "center", marginBottom: isMobile ? "24px" : "40px" }}>
        <h3 style={{ fontFamily: "'Playfair Display', serif", color: "#2D242E", fontSize: isMobile ? "24px" : "32px", marginBottom: "8px" }}>
          {content.title}
        </h3>
      ...
      <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: isMobile ? "20px" : "40px", alignItems: "center" }}>
        ...
        <div style={{ flex: 1, position: "relative", minWidth: isMobile ? "100%" : "400px" }}>
          <svg viewBox="0 0 400 320" style={{ width: "100%", height: "auto", overflow: "visible" }}>
            <defs>
              {/* Filtro per texture organica e linea "tremolante" tipo matita */}
            ...
            <g filter="url(#handDrawn)">
              {Object.keys(BRAIN_VISUALS).map((lobeKey) => {
                ...
                // TRACCIATI AD ALTA COMPLESSITÀ (Silhouette asimmetriche e ricche)
                const paths = {
                  frontal: "M140.4,46.1c-13.4-1.8-38.3,4.4-58.4,26.4c-20.1,22-29.4,59.3-15.3,83.9c14.1,24.6,35.9,34.8,61,28.8c4.3-19.1,10.6-43.2,19.2-54.6c-4.1-23.7-9.5-51.5,10.2-68C149.3,51,146.4,46.9,140.4,46.1z",
                  parietal: "M163.7,85.6c4,0,13.2,8.1,17.4,52.5c36.4,4.2,74.7-10.4,74.7-52c0-41.6-43.2-64.4-96-64.4c-11.4,0-15.5,33.1-13.2,54C151.7,80.1,159.4,84.7,163.7,85.6z",
                  temporal: "M137,210c12.1,23.5,52,28.8,72.6,13.4c12-8.9,3.7-24.8-24.6-39.7c-28.3-14.9-40.1,11-48,26.3z",
                  occipital: "M255.8,86.1c13.7,11.2,16,51.8,0,72.5c-16,20.7-50,11.3-40,0.4C225.8,138,241.8,122.2,255.8,86.1z",
                  cerebellum: "M183.1,223c16.3,0,48,4.2,48,27.1c-24.5,13.7-57.1,13.2-66.2-7.3c-4.2-11,8.3-19.8,18.2-19.8z"
                };

                // SOLCHI INTERNI (Danno l'aspetto di un cervello reale)
                const folds = {
                  frontal: "M90,75q12,18 28,6 M85,110q15,12 32,-4 M102,145q22,6 38,-12",
                  parietal: "M175,55q28,12 48,-6 M185,95q22,12 42,-6",
                  temporal: "M155,200q18,12 34,-6",
                  occipital: "M220,115q12,18 22,6",
                  cerebellum: "M185,235h35 M190,245h25"
                };
                ...
                return (
                  <g key={lobeKey} onClick={() => setActiveLobe(isActive ? null : lobeKey)} style={{ cursor: "pointer" }}>
                    <path 
                      ...
                      style={{ transition: "all 0.5s ease", transform: isActive ? "scale(1.02) translateY(-2px)" : "none" }}
                    />
                    <path 
                      d={folds[lobeKey]} 
                      fill="none" 
          ...
          <div style={{ textAlign: "center", fontSize: "12px", color: "#8E7A91", marginTop: "12px", fontStyle: "italic" }}>
            Tocca le aree per approfondire
          </div>
        </div>

        {/* TESTI INTERATTIVI */}
        <div ref={infoRef} style={{ flex: 1.2, display: "flex", flexDirection: "column", gap: "12px", width: "100%" }}>
          {Object.keys(BRAIN_VISUALS).map((lobeKey) => {
            const visual = BRAIN_VISUALS[lobeKey];
            const isActive = activeLobe === lobeKey;
            ...
            return (
              <div 
                ...
                style={{
                  ...
                  boxShadow: isActive ? `0 12px 30px ${visual.stroke}20` : "none",
...
function PreadolescenzaPage() {
  const isMobile = useIsMobile();
  const [activeTab, setActiveTab] = useState("cervello");
  ...
  const content = {
    ...
    identita: {
      ...
      cards: [
        ...
        { icon: "🌈", titolo: "Identità sessuale e di genere", testo: "Tra i 15 e i 18 anni emergono le domande più profonde sull'orientamento sessuale e sull'identità di genere. La ricerca mostra che il supporto familiare in questa fase è il fattore più predittivo del benessere psicologico a lungo termine. Il non-giudizio non è permissivismo — è protezione." },
        ...
        { icon: "🧘", titolo: "Autostima e corpo", testo: "Il corpo continua a cambiare e l'autostima è strettamente legata all'immagine corporea in questa fase. Confronto con gli standard social, ritardo puberale o puberà precoce — sono tutte fonti di stress reale. Non minimizzare le preoccupazioni sul corpo: ascoltarle è il primo passo." },
      ]
    },
    relazioni: {
    ...
    rischi: {
      ...
      cards: [
        ...
        { icon: "📱", titolo: "Dipendenza digitale", testo: "L'uso problematico dei social riguarda il 15-20% degli adolescenti italiani. I segnali: incapacità di staccarsi, umore che dipende dai like, notturne digitali frequenti, perdita di interesse per attività offline. Non è solo questione di tempo-schermo: è qualità dell'uso." },
        { icon: "🌀", titolo: "Disturbi alimentari", testo: "L'incidenza dei disturbi alimentari picca tra i 14 e i 18 anni, con aumento significativo post-Covid. Non riguardano il cibo — riguardano il controllo, l'identità, il dolore. I segnali da non ignorare: restrizione alimentare marcata, abbuffate, esercizio compulsivo, ossessione per il corpo." },
        ...
        { icon: "💙", titolo: "Salute mentale: i numeri", testo: "Il 20% degli adolescenti ha un disturbo mentale diagnosticabile — la maggior parte non riceve cura. Ansia e depressione sono le più frequenti. L'accesso precoce al supporto psicologico cambia la traiettoria di vita. Non aspettare che 'passi da solo'." },
        { icon: "🆘", titolo: "Pensieri suicidari", testo: "Se il ragazzo esprime pensieri di non voler esistere, di essere un peso per gli altri, o fa domande sul suicidio — prendi sul serio. Non lasciarlo solo, non minimizzare, non arrabbiarti. Contatta immediatamente un professionista o il numero di emergenza. Il telefono amico per i ragazzi: 19696." },
      ]
    },
    genitore: {
...
function GravidanzaPage() {
  ...
  const listaNascita = [
    { categoria: "🛏️ La cameretta", items: ["Culla o culla condivisione del letto — il condivisione del letto sicuro riduce l'ansia notturna", "Materasso rigido certificato + coprimaterasso impermeabile", "Fasciatoio con bordatura di sicurezza", "Baby monitor — indispensabile per dormire tranquilli", "Umidificatore — protegge le vie respiratorie nelle prime settimane", "Lampada notturna soffusa per i cambi di notte"] },
